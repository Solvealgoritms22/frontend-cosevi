...
export default function ReportsPage() {
    ...
    return (
        ...
        >
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 sm:gap-12 px-4">
                <div className="space-y-4">
                    <h2 className="text-4xl sm:text-5xl font-black tracking-tighter text-slate-800 leading-none">
                        {t('analyticsReports').substring(0, t('analyticsReports').lastIndexOf(' '))} <span className="text-orange-500">{t('analyticsReports').split(' ').pop()}</span>
                    </h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:px-4">
                <GlassCard elevation="sm" className="p-4 sm:p-6 lg:p-10 border-white/40 flex flex-col gap-8 relative overflow-hidden">
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 sm:gap-6 shrink-0">
                            <div className="flex items-center gap-2 px-2 sm:px-3 py-1 bg-orange-50/50 border border-orange-100/50 rounded-full">
                                <span className="size-1.5 sm:size-2 rounded-full bg-orange-500 shadow-sm" />
                                <span className="text-fluid-label font-black text-orange-600 uppercase tracking-widest">Active</span>
                            </div>
                            ...
                            <div key={i} className="flex-1 flex flex-col items-center gap-4 group">
                                <div className="w-full relative h-56">
                                    ...
                                    <motion.div
                                        ...
                                        className="absolute bottom-0 w-full bg-linear-to-t from-orange-500 to-orange-400 rounded-t-xl shadow-xl shadow-orange-500/10 group-hover:scale-y-[1.02] transition-transform origin-bottom"
                                    />
                                </div>
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-orange-500 transition-colors">W{i + 1}</span>
                            </div>
                        ))}
                    </div>
                </GlassCard>
                ...
                <GlassCard elevation="sm" className="p-4 sm:p-10 border-white/40 flex flex-col gap-8 relative overflow-hidden">
                    <h3 className="text-2xl font-black tracking-tighter text-slate-800 flex items-center gap-4 mb-8">
                        <PieChart className="text-orange-500" size={28} />
                        {t('occupancyStatus')}
                    </h3>
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row items-center justify-center gap-10 relative">
                        ...
                        <div className="flex flex-row sm:flex-col gap-3 sm:gap-4 overflow-x-auto sm:overflow-visible w-full sm:w-auto px-4 sm:px-0">
                            {[
                                ...
                                { label: 'Pending', value: guestPercent, color: 'bg-orange-500' },
                                { label: 'Other', value: staffPercent, color: 'bg-orange-300' }
                            ].map((item, idx) => (
            ...
            <div className="glass-panel border-white/60 bg-linear-to-br from-orange-500/10 to-orange-600/5 rounded-4xl p-6 sm:p-10 flex flex-col xl:flex-row items-center justify-between gap-10 shadow-2xl mx-4">
                <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-8">
                    <div className="size-16 sm:size-20 rounded-3xl bg-orange-500 flex items-center justify-center text-white shadow-2xl shadow-orange-500/30 shrink-0">
                        <Activity size={40} />
                    </div>
                ...
                <GlassButton
                    ...
                    variant="primary"
                    glow
                    className="h-16 px-10 text-lg shadow-2xl shadow-orange-500/20 w-full xl:w-auto"
                >
                    {t('reviewInsights')}
                </GlassButton>
            </div>
