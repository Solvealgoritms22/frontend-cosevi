            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-12">
                ...
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2 p-2 bg-white/40 border border-white/60 rounded-3xl shadow-sm">
                        <button onClick={() => setViewMode("GRID")} className={cn("size-12 rounded-xl flex items-center justify-center transition-all", viewMode === "GRID" ? "bg-white shadow-md text-indigo-500" : "text-slate-400 hover:text-slate-600 ")} >
                            <Grid3X3 size={20} />
                        </button>
                        <button onClick={() => setViewMode("LIST")} className={cn("size-12 rounded-xl flex items-center justify-center transition-all", viewMode === "LIST" ? "bg-white shadow-md text-indigo-500" : "text-slate-400 hover:text-slate-600 ")} >
                            <List size={20} />
                        </button>
                    </div>
                ...
                <div className="xl:col-span-8 flex flex-col gap-10">
                    <GlassCard elevation="sm" className="p-4 sm:p-10 border-white/40">
                        <div className="flex items-center justify-between mb-12">
                            ...
                            <button onClick={() => setCurrentFloor(currentFloor >= 2 ? 1 : currentFloor + 1)} className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-600 transition-colors group" >
                                                ...
                                                <tr key={space.id} onClick={() => setSelectedSpace(space)} className={cn(
                                                    ...
                                                    <td className="px-10 py-6">
                                                        <div className="flex items-center gap-4">
                                                            <div className="size-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-500 font-black text-xs border border-indigo-100 ">{space.name}</div>
                                                    ...
                                                    <td className="px-10 py-6">
                                                        <div className="flex items-center gap-3 text-xs font-bold text-slate-500 ">
                                                            <Layers size={14} className="text-indigo-400" /> {space.type}
                                                        </div>
                                                    </td>
                                                    <td className="px-10 py-6">
                                                        <div className="flex items-center gap-3 text-sm font-black text-slate-800 ">
                                                            <Car size={16} className={space.status === 'OCCUPIED' ? "text-indigo-500" : "text-slate-300 "} /> {space.vehicle || <span className="text-slate-300 font-medium">---</span>}
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))}
                        ...
                        <div className="space-y-10">
                            <div className="space-y-6">
                                ...
                                <div className="relative group">
                                    ...
                                    <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="relative z-10 w-full h-16 pl-14 pr-6 bg-white/40 border border-white/60 rounded-2xl outline-none font-bold text-slate-800 placeholder:text-slate-400 tracking-tight focus:ring-4 focus:ring-indigo-500/5 transition-all" placeholder="Spot or vehicle identity..." />
                                </div>
                            </div>
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {['ALL', 'AVAILABLE', 'OCCUPIED', 'RESERVED'].map((opt) => (
                                        <button key={opt} onClick={() => setFilter(opt)} className={cn(
                                            ...
                                            filter === opt ? "bg-indigo-50 text-indigo-600 border-indigo-100 shadow-sm" : "bg-white/30 border-transparent hover:bg-white/60 text-slate-500 "
                                        )} >
                                            <div className="flex items-center gap-4">
                ...
                <form onSubmit={handleSubmit(handleCreateSpace)} className="space-y-10 p-6">
                    <div className="space-y-4">
                        ...
                        <input {...register("name")} className={cn(
                            "w-full px-8 h-16 bg-slate-50/50 border border-slate-100 rounded-3xl text-sm font-bold text-slate-800 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:bg-white transition-all",
